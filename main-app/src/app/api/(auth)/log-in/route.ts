...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!userAuth) {
      return sendResponse('User not found, Please create an account to continue', 401);
    }

    // check if user was created with OAuth (no password)
    if (!userAuth.password) {
      const authMethod = userAuth.authMethod.charAt(0).toUpperCase() + userAuth.authMethod.slice(1);
      return sendResponse(
